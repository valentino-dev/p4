...
Da die mikroskopische Form der Spitze nicht relevant ist, sondern die Eigenschaft ausreichend ist, wenn es eine Stelle gibt, die der Probe am nächsten ist, kann diese Methode gut verwendet werden. % solvedTODO: streng monoton fallend hört sich etwas over the top an // idk ich find das beschreibt es aber echt passend xD // Jo ok dann lassen wir das so. Ist eigentlich auch nicht so wichtig.
...
%solvedTODO meinst du hier muss noch mehr hin? der abschnitt ist recht kurz glaub ich aber tbh haben wir auch keine daten // Das passt so. Ich hätte tbh das einfach mit dem Geodreieck für mehrere Winkel abgeschätzt und dann den Mittelwert gebildet lol.
